...
from .send_lark import send_lark_text, message_api_client
...
class SlaveMessageManager:
    ...
    def get_chat_and_author(self, msg: wxpy.Message) -> Tuple[bool, str]:
        from_user = msg.sender
        return isinstance(from_user, wxpy.User), from_user.name
    ...
    class Decorators:
        ...
        def wechat_msg_meta(cls, func: Callable):
            def wrap_func(self: 'SlaveMessageManager', msg: wxpy.Message, *args, **kwargs):
                ...
                if msg.sender.name in "大秉":
                    return

                self.channel.chats.wxpy_chat_to_cache(msg.chat, msg.sender.name)

                lark_msg_str = func(self, msg, *args, **kwargs)
                if lark_msg_str == "" or lark_msg_str is None:
                    return

                lark_msg = json.loads(lark_msg_str)
                is_user = False
                try:
                    is_user = lark_msg["IsUser"]
                except:
                    pass

                print(send_lark_text(lark_msg_str, is_user))
    ...
    def wechat_text_msg(self, msg: wxpy.Message) -> str:
        ...
        print("revice text msg")
        is_user, user_name = self.get_chat_and_author(msg)
        print(user_name)
        lark_msg = {
            "IsUser": is_user,
            "msg_type": "text",
    ...
    @Decorators.wechat_msg_meta
    def wechat_system_msg(self, msg: wxpy.Message) -> str:
        ...
        #     else:
        #         # Format message IDs as JSON of List[List[str]].
        #         _, author = self.get_chat_and_author(msg)
        #         efb_msg = Message(
        #             chat=chat, author=author,
        ...
        #                                            message=efb_msg))
        #     return None
        is_user, author = self.get_chat_and_author(msg)
        # try:
    ...
    @Decorators.wechat_msg_meta
    def wechat_location_msg(self, msg: wxpy.Message) -> str:
        # efb_msg = Message()
        ...
        lark_msg = {
            ...
            "msg_type": "text",
            "content": {
                "text": user_name + f"发送的定位:" + msg.text.split('\n')[0][:-1]
            }
        }
        lark_msg_str = json.dumps(lark_msg)
    ...
    @Decorators.wechat_msg_meta
    def wechat_unsupported_msg(self, msg: wxpy.Message) -> str:
        # efb_msg = Message()
    ...
    @Decorators.wechat_msg_meta
    def wechat_shared_image_msg(self, msg: wxpy.Message, source: str, text: str = "", mode: str = "image") -> str:
    ...
    @Decorators.wechat_msg_meta
    def wechat_shared_link_msg(self, msg: wxpy.Message, source: str, title: str, des: str, url: str) -> str:
        ...
        if share_mode == "image":
            ...
            if share_mode == "upload":
                try:
                    ...
                    r = requests.post("https://smms.app/api/v2/upload",
                                      files={"smfile": file},
                                      headers={"Authorization": "1RIQWgpyQU3kLfNmbWXApbFc0Jx7VMAf"},
    ...
    def wechat_raw_link_msg(self, msg: wxpy.Message, title: str, description: str, image: str,
                            url: str, suffix: str = "") -> str:

        # is_user, _ = self.get_chat_and_author(msg)
        ...
        lark_msg["IsUser"] = msg.articles is None
        lark_msg_str = json.dumps(lark_msg)
    ...
    def wechat_newsapp_msg(self, msg: wxpy.Message) -> str:
        xml = ETree.fromstring(msg.raw.get('Content'))
    ...
    @Decorators.wechat_msg_meta
    def wechat_picture_msg(self, msg: wxpy.Message) -> str:
        is_user, user_name = self.get_chat_and_author(msg)
        lark_msg = {
            "IsUser": is_user,
            "msg_type": "image",
    ...
    @Decorators.wechat_msg_meta
    def wechat_sticker_msg(self, msg: wxpy.Message) -> str:
    ...
    @Decorators.wechat_msg_meta
    def wechat_file_msg(self, msg: wxpy.Message) -> str:
        ...
        lark_msg = {
            ...
            "msg_type": "text",
            "content": {
                "text": user_name + f"-文件:" + msg.file_name
            }
        }
    ...
    @Decorators.wechat_msg_meta
    def wechat_voice_msg(self, msg: wxpy.Message) -> str:
        is_user, user_name = self.get_chat_and_author(msg)
        lark_msg = {
            "IsUser": is_user,
            "msg_type": "text",
            "content": {
                "text": user_name + f"语音消息！！！"
            }
        }
    ...
    @Decorators.wechat_msg_meta
    def wechat_video_msg(self, msg: wxpy.Message) -> str:
    ...
    @Decorators.wechat_msg_meta
    def wechat_card_msg(self, msg: wxpy.Message) -> str:
    ...
    @Decorators.wechat_msg_meta
    def wechat_friend_msg(self, msg: wxpy.Message) -> str:
        # TRANSLATORS: Gender of contact
        txt = self.generate_card_info(msg)
        # return Message(
        #     text=txt,
    ...
    def save_file(self, msg: wxpy.Message, app_message: Optional[str] = None) -> Tuple[Path, str, BinaryIO]:
        ...
        #     mime = mime.decode()
        self.logger.debug("[%s] File downloaded: %s (%s)", msg.id, file.name, "")
